// Content script for Google Meet integration (Firefox)
...
class MeetTranslator {
  ...
  async processSignLanguageFrame(canvas) {
    try {
      ...
      const liveOutput = document.getElementById('sign-live');
      liveOutput.textContent = '⚠️ Start backend: python backend-server.py';
      liveOutput.style.display = 'block';
    }
  }
  ...
  setupSpeechRecognition() {
    ...
    if (!SpeechRecognition) {
      console.warn('Speech recognition not supported in Firefox');
      return;
    }
    try {
  ...
  toggleSpeechToText() {
    ...
    if (this.speechToTextActive) {
      ...
      if (this.recognition) {
        ...
        if (liveOutput) {
          liveOutput.textContent = '⚠️ Firefox doesn\'t support Web Speech API. Use Chrome.';
          liveOutput.style.display = 'block';
        }
        status.textContent = 'OFF';
        btn.classList.remove('active');
